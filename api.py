...
async def upload_properties(file: UploadFile):
    ...
    try:
        ...
        # Debug: Print headers to see what we actually have
        ...
        for row_num, row in enumerate(csv_reader):
            ...
            # MLS Number
            mls_id = (row.get('MLS_Number') or row.get('MLS Number') or 
                     row.get('MLS_number') or row.get('mls_number') or 
                     f"PROP_{len(new_properties)}")
            
            # Address  
            address = (row.get('Address') or row.get('address') or 
                      row.get('ADDRESS') or 'Unknown Address')
            
            # City
            city = (row.get('City') or row.get('city') or 
                   row.get('CITY') or 'Unknown City')
            
            # Province
            province = (row.get('Province') or row.get('province') or 
                       row.get('PROVINCE') or 'AB')
            
            # Price - try multiple formats
            price_text = (row.get('Price') or row.get('price') or 
                         row.get('PRICE') or row.get('List Price') or 
                         row.get('List_Price') or '0')
            try:
                clean_price = str(price_text).replace('
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port), '').replace(',', '').replace(' ', '').strip()
                price = float(clean_price) if clean_price else 0
            except:
                price = 0
                
            # Property Type
            prop_type = (row.get('Property_Type') or row.get('Property Type') or 
                        row.get('property_type') or row.get('Type') or 'Unknown')
            
            # Zoning
            zoning = (row.get('Zoning') or row.get('zoning') or 
                     row.get('ZONING') or 'Unknown')
            
            # Features
            features = (row.get('Features') or row.get('features') or 
                       row.get('FEATURES') or row.get('Description') or '')
            
            # Time on Market
            time_text = (row.get('Time_on_Market') or row.get('Time on Market') or 
                        row.get('Days on Market') or row.get('DOM') or '0')
            try:
            ...
            # Agent
            agent = (row.get('Listing_Agent') or row.get('Listing Agent') or 
                    row.get('Agent') or row.get('AGENT') or 'Unknown')
            
            # Brokerage  
            brokerage = (row.get('Brokerage') or row.get('brokerage') or 
                        row.get('BROKERAGE') or row.get('Company') or 'Unknown')
            ...
            # Debug first processed property
            if row_num == 0:
